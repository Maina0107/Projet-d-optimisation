...
class VersionRayon_2(ModelesPCentre):
   ...
   def creer_modele(self, capacite: bool = False):
        ...
        K = range(len(data.distances_triees))   # u_k est défini pour k allant de 0 à K
        ...
        if (capacite == True): 
            ...
            for i in F:
                model.c6.add((quicksum(data.demandes[j] * model.y[i,j] for j in C )) <= data.capacites[i]*model.x[i])
            ...
            for i in F:
                for j in C:
                    model.c7.add( quicksum( data.distances_triees[k]*model.u[k] for k in K ) >= model.y[i,j]*data.matrice_distances[i,j])
            ...
            for i in F:
                for j in C:
                    model.c7.add( (1 - quicksum(model.u[k] for k in K if data.matrice_distances[i,j] > data.distances_triees[k] )) >= model.y[i,j])
